#!/usr/bin/env python3
"""Audit the cleaned stacking dataset before it is trained on.

Runs against whichever copy it is pointed at, so the same script checks the
a800new copy and any later copy. Read-only. Exits 1 on any hard failure.

Task-specific checks beyond the usual structure pass:
  - the cut must land after the LAST gripper release;
  - every kept episode must still show exactly ONE gripper cycle, which is what
    the source batch had; a second cycle would mean the cut moved or the wrong
    episodes were pulled in;
...
RUN = sys.argv[1] if len(sys.argv) > 1 else "/ssd/zwwl_user2/parcel_smolvla/20260907_stack_clean_50"
...
print()
print("=== 4b. 夹爪周期数(这批应该每条恰好 1 个)===")
from collections import Counter
cc = Counter(ncyc)
print("      分布:", dict(sorted(cc.items())))
check(set(ncyc) == {1}, "每条恰好 1 个夹爪周期",
      "异常的 ep %s" % [i for i, c in enumerate(ncyc) if c != 1] if set(ncyc) != {1} else "")
