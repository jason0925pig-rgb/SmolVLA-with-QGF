#!/usr/bin/env python3
"""Stack-white-box-on-purple-box cleaning, derived from the screwdriver cleaner.

Same three design decisions, with this batch's own measured facts:
...
1. The cut is [0, t_open_last + 7 s], not a mid-recording window. This task
   ...
   Measured tails before the cut: min 8.1 s, median 12.8 s, max 34.3 s, so
   there is real idle time to remove in every one of the fifty episodes.

2. The tail anchor is the LAST confirmed release. Unlike the screwdriver batch,
   ALL FIFTY episodes here have exactly one gripper cycle, so last == first and
   the two rules coincide. The last-release form is kept anyway: it is correct
   in both cases, and the cycle count is recorded per episode rather than
   assumed, so a re-grasp appearing in a future batch cannot silently change
   what the cut means.

3. J5 is CHECKED but not shifted. Measured on these fifty: per-episode mean
   spread 0.597 rad (34.2 deg), largest gap 0.097 rad, all fifty on the
   negative branch - nothing near 2*pi. The script refuses to run if that stops
   being true, rather than silently applying a correction that is not needed.
...
SRC = Path("/ssd/zwwl_user2/stack_src/lerobot_dataset")
RUN = Path("/ssd/zwwl_user2/parcel_smolvla/20260907_stack_clean_50")
CLEAN = RUN / "clean_lerobot_v3"
MAN = RUN / "manifest"
TASK = "把白色盒子叠在紫色盒子上"
OLD_EPS = list(range(250, 300))          # the whole stacking batch, contiguous
CAMS = ["observation.images.chest", "observation.images.wrist_right"]
OPEN_T, CLOSE_T, CONFIRM, TAIL_S = 0.15, 0.85, 5, 7.0
GRIP, J5, FPS = 7, 4, 30
TWO_PI = 2.0 * np.pi
# a800new has no system ffmpeg; imageio-ffmpeg ships a static build with libx264.
...
def main():
    ...
    (MAN / "cleaning_report.json").write_text(json.dumps(
        {"run_id": "stack_clean_50_20260907",
         "task_text": TASK,
         "source": str(SRC),
         "source_episodes": "250-299 (the contiguous stacking batch)",
         "cut_rule": "[0, last confirmed release + 7.0 s]; every episode in this "
                     "batch has exactly one gripper cycle, so last == first here, "
                     "and the per-episode cycle count is recorded, not assumed",
