...
# GPU 3 also hosts another user's vLLM service, which pre-allocates a fixed
# 61.6 GB block (75% utilisation) and does not grow - verified static over a
# 60 s window. That leaves 20.3 GB against the 17.4 GB this recipe peaked at
# for the green run. expandable_segments keeps allocator fragmentation from
# eating the remaining headroom.
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True

GPU=3
RUN=/ssd/zwwl_user2/parcel_smolvla/20260904_screwdriver_clean_50
DS=$RUN/clean_lerobot_v3
OUT=$RUN/outputs/train_screwdriver
INIT=~/parcel_smolvla/models/smolvla_parcel_init
REPO=local/screwdriver_into_box
...
echo "============================================================"
echo "SmolVLA fine-tune: screwdriver into box"
echo "============================================================"
...
rm -rf "$OUT"
echo "$(date '+%m-%d %H:%M:%S') TRAIN_START gpu=$GPU screwdriver" >> "$RUN/PIPELINE.log"
