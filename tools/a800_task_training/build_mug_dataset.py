#!/usr/bin/env python3
"""Build the standalone mug-task dataset from source episodes 100-149.
...
SRC = Path.home() / "mug_src/lerobot_dataset"
RUN = Path.home() / "parcel_smolvla/20260827_mug_purple_box_50_45train_5val"
DST = RUN / "mug_purple_box_lerobot_v3"
TASK = "把水杯放到紫色的箱子上"
CAMS = ["observation.images.chest", "observation.images.wrist_right"]
OLD_EPS = list(range(100, 150))
...
def main():
    ...
    m0 = pd.read_parquet(SRC / "meta/episodes/chunk-000/file-100.parquet")
    ...
    (RUN / "source_readonly_manifest/source_files_sha256.json").write_text(json.dumps({
        "source": "Orin onearm_Tele/lerobot_dataset eps 100-149, streamed via laptop 2026-08-27",
