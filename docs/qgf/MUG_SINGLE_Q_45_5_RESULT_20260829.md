...
## 附二:为什么失败 episode 上 Q 是平的 —— 根因诊断

针对附一的现象做了四个假设检验(脚本 `tools/qgf_4090_staging/diagnose_flat_q.py`,
日志 `runs/…/logs/diagnose_flat_q.log`)。**结论:Q 退化成了目标接近度探测器,
0.206 不是"预测失败",而是"还看不出来"的先验值。**

### H1 ✅ critic 在 episode 开头没有任何意见

按进度分桶(25 条,15 成功 / 10 失败):

| 进度 | 成功 Q | 失败 Q | 分离度 |
| --- | --- | --- | ---: |
| **0–20%** | 0.2076 ± 0.0067 | 0.2064 ± 0.0044 | **0.0012** |
| 20–40% | 0.4794 ± 0.2850 | 0.2418 ± 0.1260 | 0.2375 |
| 40–60% | 0.8988 ± 0.0903 | 0.2582 ± 0.1720 | 0.6406 |
| 60–80% | 0.9307 ± 0.0220 | 0.2635 ± 0.1772 | 0.6673 |
| 80–100% | 0.9488 ± 0.0228 | 0.2553 ± 0.1490 | 0.6935 |

**成功组开头也是 0.2076**,与失败组无差别。所以 0.206 是无信息状态的取值。
...
reward > 0            56  ( 1.55%)
带 done 标志          91  ( 2.52%)
纯自举 (r=0, d=0)   3515  (97.48%)
```

非接地的目标只说"和 V(s') 保持一致"。加上 γ=0.99、每 chunk 3.33 s、episode 仅
9–54 个 chunk(γ⁵⁴=0.58),**折扣本身几乎不产生时间梯度**。于是 Q 只能靠某个输入
特征预示终局,而本数据集里唯一这样的特征是"视觉上目标接近完成",它只在成功
episode 的后 40% 出现。

### H3 ❌ 假设被推翻:LayerNorm 不是元凶

原假设:`state_proj = Sequential(LayerNorm(8), Linear(8,256))` 跨 8 个关节归一化会
毁掉位姿信息。测量结果否定了它:

```
raw 两两距离均值 0.3157  →  LN 后 0.1408
两个距离矩阵相关系数     0.9986
```

LN 没有打乱位姿间的相对关系,基本只是整体缩小 2.24 倍,后面的 Linear 完全能补偿
(该数据集每样本跨关节的均值/标准差本身近乎常数,std 仅 0.0763 / 0.0688)。

真实原因更朴素:**本任务的本体感受几乎不变**。

```
J1 [-2.827, -2.164]   J4 [-2.289, -1.337]   J6 [-0.698, -0.361]   J7 [4.319, 4.778]
```

每次都是同一套接近动作,关节只在 0.3–0.8 rad 的窄带内变化;区分成败的是"杯子有没有
被放进去"这一**视觉事实**。加上 state 在序列里只占 1 个 token(视觉 128、动作 50),
梯度自然走高容量的视觉通路。

### H4 ✅ V 的先验值就是 0.206

| episode | outcome | V 均值 | V 首帧 | V 末帧 |
| --- | --- | ---: | ---: | ---: |
| ep3 | failure | 0.2142 | 0.2035 | 0.2095 |
| ep5 | failure | 0.2069 | 0.2051 | 0.2085 |
| ep10 | success | 0.6567 | **0.2060** | 0.9942 |
| ep18 | success | 0.6065 | **0.2020** | 1.0119 |
| ep36 | success | 0.7317 | **0.2056** | 0.9919 |

**五条 episode 的 V 首帧全部落在 0.202–0.206,不分成败。** Q 通过 97.5% 的自举
原样继承了这个先验。
...
```
终止奖励极稀疏(97.5% 纯自举)
  → critic 学不到"动作好不好",只能找预示终局的输入特征
  → 本任务里唯一这样的特征 = 视觉上目标接近完成(仅后 40% 出现)
  → Q 退化为目标接近度探测器
  → 失败 episode 全程停在先验 0.206(σ=0.0015);成功 episode 后半段爬到 0.96
```

同时解释了另外两个观察:第 5 节里分离度在 epoch 15 后一路衰减,是 97.5% 自举必然
带来的 OOD 动作值高估;附一里 state 通道死,是任务本身本体感受无信息量,而非归一化问题。

### 对 QGF 的影响与建议

QGF 求 ∂Q/∂a 推动作,要求 Q 在**需要干预的时段**内随动作变化。当前情况恰好相反:
引导信号最强的后 40%,成功的已在成功、失败的已经跑偏;而最需要干预的开头 20%,
Q 对一切输入都是 0.206,∂Q/∂a 只剩噪声,引导方向任意。

**这不是 critic 训坏了,是"终止奖励 + 45 条数据"这个设定的必然产物**,换超参、
换选点、多训轮次都改不掉。按杠杆排序:

1. **给中段接地** —— 加子目标/进度奖励(如"夹爪在杯子上闭合"给部分信用),直接
   提高 grounded 比例,是唯一能动摇因果链源头的改动。
2. **失败样本多样性** —— 21 条失败全是"就是没做到",critic 学不到"怎么错的";
   标注失败模式(没抓住 / 抓了掉 / 放错位置)后失败侧才有结构。
3. **上机时的便宜保护** —— 既然引导只在后段有信号,可考虑**仅当 Q 离开先验后才启用
   引导**(如 Q > 0.206 + kσ),前段跑 baseline,避免开局被任意方向的噪声梯度推歪。
   不需要重训。
4. state 通道不必修 —— 该任务的本体感受确实没有信息量。
